    ...
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Asistencia este mes</span>
                <span className="font-semibold">
                  {datosEntrenamiento.asistenciaEsteMes}/{datosEntrenamiento.totalSesiones} sesiones
                </span>
              </div>
              <Progress value={datosEntrenamiento.porcentajeAsistencia} className="h-2" />
              <p className="text-xs text-muted-foreground">
                {datosEntrenamiento.porcentajeAsistencia}% de asistencia
              </p>
            </div>
            
            <div className="pt-4 space-y-2">
              <div className="flex justify-between">
                <span className="text-sm">Horas acumuladas:</span>
                <span className="font-medium">{datosEntrenamiento.horasAcumuladas} horas</span>
              </div>
              <div className="flex justify-between">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-primary">{estadisticasImpacto.eventosRepresentados}</div>
                <p className="text-xs text-muted-foreground">Eventos UNIMET</p>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-primary">{estadisticasImpacto.clinicasDictadas}</div>
                <p className="text-xs text-muted-foreground">Clínicas dictadas</p>
              </div>
            </div>
            <div className="text-center pt-2">
              <div className="text-3xl font-bold text-primary">{estadisticasImpacto.ninosBeneficiados}</div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Eventos donde has representado a UNIMET
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {representacionInstitucional.map((evento, index) => (
              <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  <h3 className="font-medium">{evento.evento}</h3>
